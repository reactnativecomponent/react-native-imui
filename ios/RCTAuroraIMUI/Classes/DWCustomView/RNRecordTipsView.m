...
- (void)setLevel:(CGFloat)level{
    ...
    if (self.status == UIRecordSoundStatusRecoding) {
        ...
        rightImage.image = [UIImage nim_fetchView:imgName];
    }
}
...
- (NSString *)dealwithLevel:(CGFloat )level{
    ...
    return [NSString stringWithFormat:@"/viewImgs/%@",imgName];
}
...
- (void)showViewWithRecording{
    ...
    if (([_time integerValue] >0)&&([_time integerValue] <10)){
        ...
        leftImage.image = [UIImage nim_fetchView:@"/viewImgs/recorder"];
        rightImage.image = [UIImage nim_fetchView:[self dealwithLevel:_level]];
        leftImage.hidden = NO;
...
- (void)showViewWithCancle{
    ...
    statusImage.image = [UIImage nim_fetchView:@"/viewImgs/cancel"];
    leftImage.hidden = YES;
    rightImage.hidden = YES;
...
- (void)showVieWithTimeShort{
    ...
    statusImage.image = [UIImage nim_fetchView:@"/viewImgs/voice_to_short"];
    leftImage.hidden = YES;
    rightImage.hidden = YES;
    statusImage.hidden = NO;
    tipsNumLabel.hidden = YES;
