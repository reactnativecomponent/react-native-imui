...
@implementation DWTextEffectWindow

+ (instancetype)sharedWindow {
    static DWTextEffectWindow *one = nil;
    if (one == nil) {
        // iOS 9 compatible
    ...
    dispatch_once(&onceToken, ^{
        if (!DWTextIsAppExtension()) {
            one = [self new];
            one.frame = (CGRect){.size = DWTextScreenSize()};
...
- (void)becomeKeyWindow {
    [[DWTextSharedApplication().delegate window] makeKeyWindow];
}

- (UIViewController *)rootViewController {
    for (UIWindow *window in [DWTextSharedApplication() windows]) {
...
- (DWTextDirection)_keyboardDirection {
    CGRect keyboardFrame = [DWTextKeyboardManager defaultManager].keyboardFrame;
    keyboardFrame = [[DWTextKeyboardManager defaultManager] convertRect:keyboardFrame toView:self];
    if (CGRectIsNull(keyboardFrame) || CGRectIsEmpty(keyboardFrame)) return DWTextDirectionNone;
    
    if (CGRectGetMinY(keyboardFrame) == 0 &&
        CGRectGetMinX(keyboardFrame) == 0 &&
        CGRectGetMaxX(keyboardFrame) == CGRectGetWidth(self.frame))
        return DWTextDirectionTop;
    ...
    if (CGRectGetMaxX(keyboardFrame) == CGRectGetWidth(self.frame) &&
        ...
        return DWTextDirectionRight;
    
    if (CGRectGetMaxY(keyboardFrame) == CGRectGetHeight(self.frame) &&
        CGRectGetMinX(keyboardFrame) == 0 &&
        CGRectGetMaxX(keyboardFrame) == CGRectGetWidth(self.frame))
        return DWTextDirectionBottom;
    
    if (CGRectGetMinX(keyboardFrame) == 0 &&
        CGRectGetMinY(keyboardFrame) == 0 &&
        CGRectGetMaxY(keyboardFrame) == CGRectGetHeight(self.frame))
        return DWTextDirectionLeft;
    
    return DWTextDirectionNone;
}
...
- (CGPoint)_correctedCenter:(CGPoint)center forMagnifier:(DWTextMagnifier *)mag rotation:(CGFloat)rotation {
    CGFloat degree = DWTextRadiansToDegrees(rotation);
    ...
    if (degree <= 1 || degree >= 7) { //top
        if (mag.type == DWTextMagnifierTypeCaret) {
            if (center.y < caretExt)
                center.y = caretExt;
        } else if (mag.type == DWTextMagnifierTypeRanged) {
 ...
 */
- (CGFloat)_updateMagnifier:(DWTextMagnifier *)mag {
    UIApplication *app = DWTextSharedApplication();
    if (!app) return 0;
    ...
    CGPoint captureCenter = [self DW_convertPoint:mag.hostCaptureCenter fromViewOrWindow:hostView];
    ...
    CGAffineTransform trans = DWTextCGAffineTransformGetFromViews(hostView, self);
    CGFloat rotation = DWTextCGAffineTransformGetRotation(trans);
    ...
    for (UIWindow *window in windows) {
        ...
        CGContextConcatCTM(context, DWTextCGAffineTransformGetFromViews(window, self));
...
- (void)showMagnifier:(DWTextMagnifier *)mag {
    ...
    CGPoint center = [self DW_convertPoint:mag.hostPopoverCenter fromViewOrWindow:mag.hostView];
    CGAffineTransform trans = CGAffineTransformMakeRotation(rotation);
    trans = CGAffineTransformScale(trans, 0.3, 0.3);
    mag.transform = trans;
    mag.center = center;
    if (mag.type == DWTextMagnifierTypeRanged) {
        mag.alpha = 0;
    }
    NSTimeInterval time = mag.type == DWTextMagnifierTypeCaret ? 0.08 : 0.1;
...
- (void)hideMagnifier:(DWTextMagnifier *)mag {
    ...
    CGFloat rotation = [self _updateMagnifier:mag];
    CGPoint center = [self DW_convertPoint:mag.hostPopoverCenter fromViewOrWindow:mag.hostView];
    NSTimeInterval time = mag.type == DWTextMagnifierTypeCaret ? 0.20 : 0.15;
    [UIView animateWithDuration:time delay:0 options:UIViewAnimationOptionCurveEaseInOut | UIViewAnimationOptionAllowUserInteraction | UIViewAnimationOptionBeginFromCurrentState animations:^{
...
- (void)_updateSelectionGrabberDot:(DWSelectionGrabberDot *)dot selection:(DWTextSelectionView *)selection{
    dot.mirror.hidden = YES;
    if (selection.hostView.clipsToBounds == YES && dot.DW_visibleAlpha > 0.1) {
        CGRect dotRect = [dot DW_convertRect:dot.bounds toViewOrWindow:self];
        ...
        if (!dotInKeyboard) {
            ...
            if (DWTextCGRectGetArea(intersection) < DWTextCGRectGetArea(dotRect)) {
                CGFloat dist = DWTextCGPointGetDistanceToRect(DWTextCGRectGetCenter(dotRect), hostRect);
    ...
    CGPoint center = [dot DW_convertPoint:CGPointMake(CGRectGetWidth(dot.frame) / 2, CGRectGetHeight(dot.frame) / 2) toViewOrWindow:self];
