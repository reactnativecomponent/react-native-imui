...
const CGSize DWTextContainerMaxSize = (CGSize){0x100000, 0x100000};
...
static inline CGSize DWTextClipCGSize(CGSize size) {
    if (size.width > DWTextContainerMaxSize.width) size.width = DWTextContainerMaxSize.width;
    if (size.height > DWTextContainerMaxSize.height) size.height = DWTextContainerMaxSize.height;
    return size;
}
 ...
 */
static CGColorRef DWTextGetCGColor(CGColorRef color) {
...
@implementation DWTextLinePositionSimpleModifier
- (void)modifyLines:(NSArray *)lines fromText:(NSAttributedString *)text inContainer:(DWTextContainer *)container {
    if (container.verticalForm) {
        for (NSUInteger i = 0, max = lines.count; i < max; i++) {
            DWTextLine *line = lines[i];
...
- (id)copyWithZone:(NSZone *)zone {
    DWTextLinePositionSimpleModifier *one = [self.class new];
...
@implementation DWTextContainer {
    @package
    BOOL _readonly; ///< used only in DWTextLayout.implementation
    ...
    DWTextTruncationType _truncationType;
    NSAttributedString *_truncationToken;
    id<DWTextLinePositionModifier> _linePositionModifier;
}
...
+ (instancetype)containerWithSize:(CGSize)size insets:(UIEdgeInsets)insets {
    DWTextContainer *one = [self new];
    one.size = DWTextClipCGSize(size);
    one.insets = insets;
    return one;
}
...
- (id)copyWithZone:(NSZone *)zone {
    DWTextContainer *one = [self.class new];
...
reason:@"Cannot change the property of the 'container' in 'DWTextLayout'." userInfo:nil]; \
return; \
} \
dispatch_semaphore_wait(_lock, DISPATCH_TIME_FOREVER); \
__VA_ARGS__; \
...
- (void)setSize:(CGSize)size {
    Setter(if(!_path) _size = DWTextClipCGSize(size));
}
...
- (DWTextTruncationType)truncationType {
    Getter(DWTextTruncationType type = _truncationType) return type;
}

- (void)setTruncationType:(DWTextTruncationType)truncationType {
...
- (id<DWTextLinePositionModifier>)linePositionModifier {
    Getter(id<DWTextLinePositionModifier> m = _linePositionModifier) return m;
}
...
@interface DWTextLayout ()

@property (nonatomic, readwrite) DWTextContainer *container;
...
@property (nonatomic, readwrite) DWTextLine *truncatedLine;
...
@property (nonatomic, assign) DWRowEdge *lineRowsEdge; ///< top-left origin

@end



@implementation DWTextLayout
...
+ (DWTextLayout *)layoutWithContainerSize:(CGSize)size text:(NSAttributedString *)text {
    DWTextContainer *container = [DWTextContainer containerWithSize:size];
    return [self layoutWithContainer:container text:text];
}

+ (DWTextLayout *)layoutWithContainer:(DWTextContainer *)container text:(NSAttributedString *)text {
    return [self layoutWithContainer:container text:text range:NSMakeRange(0, text.length)];
}

+ (DWTextLayout *)layoutWithContainer:(DWTextContainer *)container text:(NSAttributedString *)text range:(NSRange)range {
    DWTextLayout *layout = NULL;
    ...
    DWTextLine *truncatedLine = nil;
    DWRowEdge *lineRowsEdge = NULL;
    ...
    if (needFixJoinedEmojiBug) {
        [((NSMutableAttributedString *)text) DW_setClearColorToJoinedEmoji];
    }
    
    layout = [[DWTextLayout alloc] _init];
    layout.text = text;
    ...
    if (container.path == nil && container.exclusionPaths.count == 0) {
        ...
        if (needFixLayoutSizeBug) {
            ...
            if (container.isVerticalForm) {
                rect.size.width = DWTextContainerMaxSize.width;
            } else {
                rect.size.height = DWTextContainerMaxSize.height;
            }
        }
    ...
    ctFrame = CTFramesetterCreateFrame(ctSetter, DWTextCFRangeFromNSRange(range), cgPath, (CFTypeRef)frameAttrs);
    ...
    for (NSUInteger i = 0; i < lineCount; i++) {
        ...
        DWTextLine *line = [DWTextLine lineWithCTLine:ctLine position:position vertical:isVerticalForm];
    ...
    if (rowCount > 0) {
        if (maximumNumberOfRows > 0) {
            if (rowCount > maximumNumberOfRows) {
                ...
                do {
                    DWTextLine *line = lines.lastObject;
                    if (!line) break;
        ...
        DWTextLine *lastLine = lines.lastObject;
        ...
        lineRowsEdge = calloc(rowCount, sizeof(DWRowEdge));
        ...
        for (NSUInteger i = 0, max = lines.count; i < max; i++) {
            ...
            if ((NSInteger)line.row != lastRowIdx) {
                if (lastRowIdx >= 0) {
                    lineRowsEdge[lastRowIdx] = (DWRowEdge) {.head = lastHead, .foot = lastFoot };
                }
                lastRowIdx = line.row;
        ...
        for (NSUInteger i = 1; i < rowCount; i++) {
            DWRowEdge v0 = lineRowsEdge[i - 1];
            DWRowEdge v1 = lineRowsEdge[i];
    ...
    { // calculate bounding size
        ...
        if (container.path) {
            ...
            rect = UIEdgeInsetsInsetRect(rect,DWTextUIEdgeInsetsInvert(container.insets));
        }
        rect = CGRectStandardize(rect);
    ...
    visibleRange = DWTextNSRangeFromCFRange(CTFrameGetVisibleStringRange(ctFrame));
    if (needTruncation) {
        DWTextLine *lastLine = lines.lastObject;
        ...
        if (container.truncationType != DWTextTruncationTypeNone) {
            ...
            if (container.truncationToken) {
                ...
                truncationToken = [[NSAttributedString alloc] initWithString:DWTextTruncationToken attributes:attrs];
                truncationTokenLine = CTLineCreateWithAttributedString((CFAttributedStringRef)truncationToken);
            }
            if (truncationTokenLine) {
                CTLineTruncationType type = kCTLineTruncationEnd;
                if (container.truncationType == DWTextTruncationTypeStart) {
                    type = kCTLineTruncationStart;
                } else if (container.truncationType == DWTextTruncationTypeMiddle) {
                ...
                if (ctLastLineExtend) {
                    ...
                    if (ctTruncatedLine) {
                        truncatedLine = [DWTextLine lineWithCTLine:ctTruncatedLine position:lastLine.position vertical:isVerticalForm];
    ...
    if (isVerticalForm) {
        NSCharacterSet *rotateCharset = DWTextVerticalFormRotateCharacterSet();
        NSCharacterSet *rotateMoveCharset = DWTextVerticalFormRotateAndMoveCharacterSet();
        
        void (^lineBlock)(DWTextLine *) = ^(DWTextLine *line){
            ...
            for (NSUInteger r = 0; r < runCount; r++) {
                ...
                BOOL isColorGlyph = DWTextCTFontContainsColorBitmapGlyphs(font);
                
                NSUInteger prevIdx = 0;
                DWTextRunGlyphDrawMode prevMode = DWTextRunGlyphDrawModeHorizontal;
                ...
                for (NSUInteger g = 0; g < glyphCount; g++) {
                    ...
                    DWTextRunGlyphDrawMode mode = glyphRotateMove ? DWTextRunGlyphDrawModeVerticalRotateMove : (glyphRotate ? DWTextRunGlyphDrawModeVerticalRotate : DWTextRunGlyphDrawModeHorizontal);
                    if (g == 0) {
                        prevMode = mode;
                    } else if (mode != prevMode) {
                        DWTextRunGlyphRange *aRange = [DWTextRunGlyphRange rangeWithRange:NSMakeRange(prevIdx, g - prevIdx) drawMode:prevMode];
                ...
                if (prevIdx < glyphCount) {
                    DWTextRunGlyphRange *aRange = [DWTextRunGlyphRange rangeWithRange:NSMakeRange(prevIdx, glyphCount - prevIdx) drawMode:prevMode];
                    [runRanges addObject:aRange];
                }
                
            }
        };
        for (DWTextLine *line in lines) {
            lineBlock(line);
        }
    ...
    if (visibleRange.length > 0) {
        ...
        void (^block)(NSDictionary *attrs, NSRange range, BOOL *stop) = ^(NSDictionary *attrs, NSRange range, BOOL *stop) {
            if (attrs[DWTextHighlightAttributeName]) layout.containsHighlight = YES;
            if (attrs[DWTextBlockBorderAttributeName]) layout.needDrawBlockBorder = YES;
            if (attrs[DWTextBackgroundBorderAttributeName]) layout.needDrawBackgroundBorder = YES;
            if (attrs[DWTextShadowAttributeName] || attrs[NSShadowAttributeName]) layout.needDrawShadow = YES;
            if (attrs[DWTextUnderlineAttributeName]) layout.needDrawUnderline = YES;
            if (attrs[DWTextAttachmentAttributeName]) layout.needDrawAttachment = YES;
            if (attrs[DWTextInnerShadowAttributeName]) layout.needDrawInnerShadow = YES;
            if (attrs[DWTextStrikethroughAttributeName]) layout.needDrawStrikethrough = YES;
            if (attrs[DWTextBorderAttributeName]) layout.needDrawBorder = YES;
        };
    ...
    for (NSUInteger i = 0, max = lines.count; i < max; i++) {
        ...
        if (line.attachments.count > 0) {
            ...
            for (DWTextAttachment *attachment in line.attachments) {
...
+ (NSArray *)layoutWithContainers:(NSArray *)containers text:(NSAttributedString *)text range:(NSRange)range {
    ...
    for (NSUInteger i = 0, max = containers.count; i < max; i++) {
        DWTextContainer *container = containers[i];
        DWTextLayout *layout = [self layoutWithContainer:container text:text range:range];
...
- (void)encodeWithCoder:(NSCoder *)aCoder {
    NSData *textData = [DWTextArchiver archivedDataWithRootObject:_text];
...
- (id)initWithCoder:(NSCoder *)aDecoder {
    ...
    NSAttributedString *text = [DWTextUnarchiver unarchiveObjectWithData:textData];
    DWTextContainer *container = [aDecoder decodeObjectForKey:@"container"];
...
- (NSUInteger)_rowIndexForEdge:(CGFloat)edge {
    ...
    while (lo <= hi) {
        mid = (lo + hi) / 2;
        DWRowEdge oneEdge = _lineRowsEdge[mid];
        if (isVertical ?
 ...
 */
- (CTRunRef)_runForLine:(DWTextLine *)line position:(DWTextPosition *)position {
    if (!line || !position) return NULL;
    CFArrayRef runs = CTLineGetGlyphRuns(line.CTLine);
    for (NSUInteger i = 0, max = CFArrayGetCount(runs); i < max; i++) {
        CTRunRef run = CFArrayGetValueAtIndex(runs, i);
        CFRange range = CTRunGetStringRange(run);
        if (position.affinity == DWTextAffinityBackward) {
...
/**
 ...
 @param block    The block to be executed before returns YES.
            ...
            next:  right position
 */
- (BOOL)_insideComposedCharacterSequences:(DWTextLine *)line position:(NSUInteger)position block:(void (^)(CGFloat left, CGFloat right, NSUInteger prev, NSUInteger next))block {
 ...
 */
- (BOOL)_insideEmoji:(DWTextLine *)line position:(NSUInteger)position block:(void (^)(CGFloat left, CGFloat right, NSUInteger prev, NSUInteger next))block {
    if (!line) return NO;
    CFArrayRef runs = CTLineGetGlyphRuns(line.CTLine);
    for (NSUInteger r = 0, rMax = CFArrayGetCount(runs); r < rMax; r++) {
        ...
        if (!DWTextCTFontContainsColorBitmapGlyphs(font)) continue;
...
/**
 ...
 @return YES if RTL.
 */
- (BOOL)_isRightToLeftInLine:(DWTextLine *)line atPoint:(CGPoint)point {
    if (!line) return NO;
    // get write direction
    BOOL RTL = NO;
    CFArrayRef runs = CTLineGetGlyphRuns(line.CTLine);
 ...
 */
- (DWTextRange *)_correctedRangeWithEdge:(DWTextRange *)range {
    NSRange visibleRange = self.visibleRange;
    DWTextPosition *start = range.start;
    DWTextPosition *end = range.end;
    
    if (start.offset == visibleRange.location && start.affinity == DWTextAffinityBackward) {
        start = [DWTextPosition positionWithOffset:start.offset affinity:DWTextAffinityForward];
    }
    
    if (end.offset == visibleRange.location + visibleRange.length && start.affinity == DWTextAffinityForward) {
        end = [DWTextPosition positionWithOffset:end.offset affinity:DWTextAffinityBackward];
    }
    
    if (start != range.start || end != range.end) {
        range = [DWTextRange rangeWithStart:start end:end];
    }
    return range;
}
...
- (NSUInteger)rowIndexForLine:(NSUInteger)line {
    ...
    return ((DWTextLine *)_lines[line]).row;
}
...
- (NSUInteger)lineIndexForPoint:(CGPoint)point {
    ...
    for (NSUInteger i = lineIdx0; i <= lineIdx1; i++) {
        CGRect bounds = ((DWTextLine *)_lines[i]).bounds;
...
- (CGFloat)offsetForTextPosition:(NSUInteger)position lineIndex:(NSUInteger)lineIndex {
    ...
    DWTextLine *line = _lines[lineIndex];
...
- (NSUInteger)textPositionForPoint:(CGPoint)point lineIndex:(NSUInteger)lineIndex {
    ...
    for (NSUInteger r = 0, max = CFArrayGetCount(runs); r < max; r++) {
        ...
        if (range.location <= idx && idx < range.location + range.length) {
            ...
            CFDictionaryRef attrs = CTRunGetAttributes(run);
            CTFontRef font = CFDictionaryGetValue(attrs, kCTFontAttributeName);
            if (!DWTextCTFontContainsColorBitmapGlyphs(font)) break;
...
- (DWTextPosition *)closestPositionToPoint:(CGPoint)point {
    ...
    if (position <= _visibleRange.location) {
        return [DWTextPosition positionWithOffset:_visibleRange.location affinity:DWTextAffinityForward];
    } else if (position >= _visibleRange.location + _visibleRange.length) {
        return [DWTextPosition positionWithOffset:_visibleRange.location + _visibleRange.length affinity:DWTextAffinityBackward];
    }
    
    DWTextAffinity finalAffinity = DWTextAffinityForward;
    BOOL finalAffinityDetected = NO;
    
    // binding range
    NSRange bindingRange;
    DWTextBinding *binding = [_text attribute:DWTextBindingAttributeName atIndex:position longestEffectiveRange:&bindingRange inRange:NSMakeRange(0, _text.length)];
    if (binding && bindingRange.length > 0) {
        NSUInteger headLineIdx = [self lineIndexForPosition:[DWTextPosition positionWithOffset:bindingRange.location]];
        NSUInteger tailLineIdx = [self lineIndexForPosition:[DWTextPosition positionWithOffset:bindingRange.location + bindingRange.length affinity:DWTextAffinityBackward]];
            ...
            if (left != CGFLOAT_MAX && right != CGFLOAT_MAX) {
                if (_container.isVerticalForm) {
                    if (fabs(point.y - left) < fabs(point.y - right)) {
                        ...
                        finalAffinity = DWTextAffinityForward;
                    } else {
                        position = bindingRange.location + bindingRange.length;
                        finalAffinity = DWTextAffinityBackward;
                    }
                } else {
    ...
    if (line.range.length == 0) {
        ...
        return [DWTextPosition positionWithOffset:line.range.location affinity:behind ? DWTextAffinityBackward:DWTextAffinityForward];
    }
    ...
    if (line.range.length <= 2) {
        ...
        if (DWTextIsLinebreakString(str)) { // an empty line ("\r", "\n", "\r\n")
            return [DWTextPosition positionWithOffset:line.range.location];
        }
    }
    ...
    if (position >= line.range.location + line.range.length - 1) {
        if (position > line.range.location) {
            ...
            if (DWTextIsLinebreakChar(c1)) {
                position--;
                if (position > line.range.location) {
                    unichar c0 = [_text.string characterAtIndex:position - 1];
                    if (DWTextIsLinebreakChar(c0)) {
                        position--;
                    }
                }
            }
    ...
    if (position == line.range.location) {
        return [DWTextPosition positionWithOffset:position];
    }
    if (position == line.range.location + line.range.length) {
        return [DWTextPosition positionWithOffset:position affinity:DWTextAffinityBackward];
    }
    ...
    if (!finalAffinityDetected) {
        ...
        if (ofs != CGFLOAT_MAX) {
            ...
            if (position >= line.range.location + line.range.length) {
                finalAffinity = RTL ? DWTextAffinityForward : DWTextAffinityBackward;
            } else if (position <= line.range.location) {
                finalAffinity = RTL ? DWTextAffinityBackward : DWTextAffinityForward;
            } else {
                finalAffinity = (ofs < (isVertical ? point.y : point.x) && !RTL) ? DWTextAffinityForward : DWTextAffinityBackward;
            }
        }
    }
    
    return [DWTextPosition positionWithOffset:position affinity:finalAffinity];
}

- (DWTextPosition *)positionForPoint:(CGPoint)point
                         oldPosition:(DWTextPosition *)oldPosition
                       otherPosition:(DWTextPosition *)otherPosition {
    if (!oldPosition || !otherPosition) {
        return oldPosition;
    }
    DWTextPosition *newPos = [self closestPositionToPoint:point];
    ...
    DWTextLine *line = _lines[lineIndex];
    DWRowEdge vertical = _lineRowsEdge[line.row];
    if (_container.verticalForm) {
    ...
    if (_container.isVerticalForm) {
        if ([oldPosition compare:otherPosition] == NSOrderedAscending) { // search backward
            DWTextRange *range = [self textRangeByExtendingPosition:otherPosition inDirection:UITextLayoutDirectionUp offset:1];
            if (range) return range.start;
        } else { // search forward
            DWTextRange *range = [self textRangeByExtendingPosition:otherPosition inDirection:UITextLayoutDirectionDown offset:1];
            if (range) return range.end;
        }
    } else {
        if ([oldPosition compare:otherPosition] == NSOrderedAscending) { // search backward
            DWTextRange *range = [self textRangeByExtendingPosition:otherPosition inDirection:UITextLayoutDirectionLeft offset:1];
            if (range) return range.start;
        } else { // search forward
            DWTextRange *range = [self textRangeByExtendingPosition:otherPosition inDirection:UITextLayoutDirectionRight offset:1];
            if (range) return range.end;
        }
    }
...
- (DWTextRange *)textRangeAtPoint:(CGPoint)point {
    ...
    DWTextPosition *pos = [self closestPositionToPoint:point];
    if (!pos) return nil;
    ...
    if (_container.verticalForm) {
        DWTextRange *range = [self textRangeByExtendingPosition:pos inDirection:(rect.origin.y >= point.y && !RTL) ? UITextLayoutDirectionUp:UITextLayoutDirectionDown offset:1];
        return range;
    } else {
        DWTextRange *range = [self textRangeByExtendingPosition:pos inDirection:(rect.origin.x >= point.x && !RTL) ? UITextLayoutDirectionLeft:UITextLayoutDirectionRight offset:1];
        return range;
    }
}

- (DWTextRange *)closestTextRangeAtPoint:(CGPoint)point {
    DWTextPosition *pos = [self closestPositionToPoint:point];
    if (!pos) return nil;
    ...
    DWTextRange *range = [self textRangeByExtendingPosition:pos inDirection:direction offset:1];
    return range;
}

- (DWTextRange *)textRangeByExtendingPosition:(DWTextPosition *)position {
    ...
    if (position.offset == visibleStart) {
        return [DWTextRange rangeWithRange:NSMakeRange(position.offset, 0)];
    } else if (position.offset == visibleEnd) {
        return [DWTextRange rangeWithRange:NSMakeRange(position.offset, 0) affinity:DWTextAffinityBackward];
    }
    
    // binding range
    NSRange tRange;
    DWTextBinding *binding = [_text attribute:DWTextBindingAttributeName atIndex:position.offset longestEffectiveRange:&tRange inRange:_visibleRange];
    if (binding && tRange.length > 0 && tRange.location < position.offset) {
        return [DWTextRange rangeWithRange:tRange];
    }
    ...
    if (lineIndex != NSNotFound) {
        ...
        if (emoji || seq) {
            return [DWTextRange rangeWithRange:NSMakeRange(_prev, _next - _prev)];
        }
    }
    ...
    if (position.offset > visibleStart && position.offset < visibleEnd) {
        ...
        if ((c0 == '\r') && position.offset < visibleEnd) {
            ...
            if (c1 == '\n') {
                return [DWTextRange rangeWithStart:[DWTextPosition positionWithOffset:position.offset - 1] end:[DWTextPosition positionWithOffset:position.offset + 1]];
            }
        }
        if (DWTextIsLinebreakChar(c0) && position.affinity == DWTextAffinityBackward) {
            NSString *str = [_text.string substringToIndex:position.offset];
            NSUInteger len = DWTextLinebreakTailLength(str);
            return [DWTextRange rangeWithStart:[DWTextPosition positionWithOffset:position.offset - len] end:[DWTextPosition positionWithOffset:position.offset]];
        }
    }
    
    return [DWTextRange rangeWithRange:NSMakeRange(position.offset, 0) affinity:position.affinity];
}
...
- (DWTextRange *)textRangeByExtendingPosition:(DWTextPosition *)position
    ...
    if (!forwardMove && position.offset == visibleStart) {
        return [DWTextRange rangeWithRange:NSMakeRange(_visibleRange.location, 0)];
    } else if (forwardMove && position.offset == visibleEnd) {
        return [DWTextRange rangeWithRange:NSMakeRange(position.offset, 0) affinity:DWTextAffinityBackward];
    }
    
    // extend from position
    DWTextRange *fromRange = [self textRangeByExtendingPosition:position];
    if (!fromRange) return nil;
    DWTextRange *allForward = [DWTextRange rangeWithStart:fromRange.start end:[DWTextPosition positionWithOffset:visibleEnd]];
    DWTextRange *allBackward = [DWTextRange rangeWithStart:[DWTextPosition positionWithOffset:visibleStart] end:fromRange.end];
    ...
    if (verticalMove) { // up/down in text layout
        ...
        DWTextLine *mostLeftLine = nil, *mostRightLine = nil;
        ...
        DWTextLine *insideLine = _lines[insideIndex];
        NSUInteger pos;
        if (isVerticalForm) {
        ...
        DWTextPosition *extPos;
        if (afinityEdge) {
            if (pos == insideLine.range.location + insideLine.range.length) {
                NSString *subStr = [_text.string substringWithRange:insideLine.range];
                NSUInteger lineBreakLen = DWTextLinebreakTailLength(subStr);
                extPos = [DWTextPosition positionWithOffset:pos - lineBreakLen];
            } else {
                extPos = [DWTextPosition positionWithOffset:pos];
            }
        } else {
            extPos = [DWTextPosition positionWithOffset:pos];
        }
        DWTextRange *ext = [self textRangeByExtendingPosition:extPos];
        if (!ext) return nil;
        if (forwardMove) {
            return [DWTextRange rangeWithStart:fromRange.start end:ext.end];
        } else {
            return [DWTextRange rangeWithStart:ext.start end:fromRange.end];
        }
        
    } else { // left/right in text layout
        DWTextPosition *toPosition = [DWTextPosition positionWithOffset:position.offset + (forwardMove ? offset : -offset)];
        if (toPosition.offset <= visibleStart) return allBackward;
        else if (toPosition.offset >= visibleEnd) return allForward;
        
        DWTextRange *toRange = [self textRangeByExtendingPosition:toPosition];
        ...
        return [DWTextRange rangeWithRange:NSMakeRange(start, end - start)];
    }
}

- (NSUInteger)lineIndexForPosition:(DWTextPosition *)position {
...
- (CGPoint)linePositionForPosition:(DWTextPosition *)position {
    NSUInteger lineIndex = [self lineIndexForPosition:position];
...
- (CGRect)firstRectForRange:(DWTextRange *)range {
    ...
    DWTextLine *startLine = _lines[startLineIndex];
    DWTextLine *endLine = _lines[endLineIndex];
    NSMutableArray *lines = [NSMutableArray new];
    for (NSUInteger i = startLineIndex; i <= startLineIndex; i++) {
        DWTextLine *line = _lines[i];
    ...
    if (_container.verticalForm) {
        if (lines.count == 1) {
            ...
            if (top > bottom) DWTEXT_SWAP(top, bottom);
        ...
        if (lines.count == 1) {
            ...
            if (left > right) DWTEXT_SWAP(left, right);
...
- (CGRect)rectForRange:(DWTextRange *)range {
    NSArray *rects = [self selectionRectsForRange:range];
    if (rects.count == 0) return CGRectNull;
    CGRect rectUnion = ((DWTextSelectionRect *)rects.firstObject).rect;
    for (NSUInteger i = 1; i < rects.count; i++) {
        DWTextSelectionRect *rect = rects[i];
...
- (NSArray *)selectionRectsForRange:(DWTextRange *)range {
    ...
    if (startLineIndex > endLineIndex) DWTEXT_SWAP(startLineIndex, endLineIndex);
    DWTextLine *startLine = _lines[startLineIndex];
    DWTextLine *endLine = _lines[endLineIndex];
    ...
    DWTextSelectionRect *start = [DWTextSelectionRect new];
    if (isVertical) {
    ...
    DWTextSelectionRect *end = [DWTextSelectionRect new];
    if (isVertical) {
    ...
    if (startLine.row == endLine.row) { // same row
        if (offsetStart > offsetEnd) DWTEXT_SWAP(offsetStart, offsetEnd);
        DWTextSelectionRect *rect = [DWTextSelectionRect new];
        if (isVertical) {
        ...
        DWTextSelectionRect *topRect = [DWTextSelectionRect new];
        ...
        DWTextSelectionRect *bottomRect = [DWTextSelectionRect new];
...
- (NSArray *)selectionRectsWithoutStartAndEndForRange:(DWTextRange *)range {
...
- (NSArray *)selectionRectsWithOnlyStartAndEndForRange:(DWTextRange *)range {
    NSMutableArray *rects = [self selectionRectsForRange:range].mutableCopy;
    for (NSInteger i = 0, max = rects.count; i < max; i++) {
        DWTextSelectionRect *rect = rects[i];
        if (!rect.containsStart && !rect.containsEnd) {
...
typedef NS_OPTIONS(NSUInteger, DWTextDecorationType) {
    DWTextDecorationTypeUnderline     = 1 << 0,
    DWTextDecorationTypeStrikethrough = 1 << 1,
};

typedef NS_OPTIONS(NSUInteger, DWTextBorderType) {
    DWTextBorderTypeBackgound = 1 << 0,
    DWTextBorderTypeNormal    = 1 << 1,
};

static CGRect DWTextMergeRectInSameLine(CGRect rect1, CGRect rect2, BOOL isVertical) {
    if (isVertical) {
...
static void DWTextGetRunsMaxMetric(CFArrayRef runs, CGFloat *xHeight, CGFloat *underlinePosition, CGFloat *lineThickness) {
...
static void DWTextDrawRun(DWTextLine *line, CTRunRef run, CGContextRef context, CGSize size, BOOL isVertical, NSArray *runRanges, CGFloat verticalOffset) {
    CGAffineTransform runTextMatrix = CTRunGetTextMatrix(run);
    BOOL runTextMatrixIsID = CGAffineTransformIsIdentity(runTextMatrix);
    
    CFDictionaryRef runAttrs = CTRunGetAttributes(run);
    NSValue *glyphTransformValue = CFDictionaryGetValue(runAttrs, (__bridge const void *)(DWTextGlyphTransformAttributeName));
    if (!isVertical && !glyphTransformValue) { // draw run
        ...
        fillColor = DWTextGetCGColor(fillColor);
        ...
        CGContextSaveGState(context); {
            ...
            if (isVertical) {
                ...
                for (DWTextRunGlyphRange *oneRange in runRanges) {
                    NSRange range = oneRange.glyphRangeInRun;
                    NSUInteger rangeMax = range.location + range.length;
                    DWTextRunGlyphDrawMode mode = oneRange.drawMode;
                    ...
                    for (NSUInteger g = range.location; g < rangeMax; g++) {
                        CGContextSaveGState(context); {
                            ...
                            if (mode) { // CJK glyph, need rotated
                                ...
                                if (mode == DWTextRunGlyphDrawModeVerticalRotateMove) {
                                    x += w;
                                    y += w;
                                }
                                CGContextSetTextPosition(context, x, y);
                            } else {
                                CGContextRotateCTM(context, DWTextDegreesToRadians(-90));
                                CGContextSetTextPosition(context,
                                                         line.position.y - size.height + glyphPositions[g].x,
                                                         line.position.x + verticalOffset + glyphPositions[g].y);
                            }
                            
                            if (DWTextCTFontContainsColorBitmapGlyphs(runFont)) {
...
static void DWTextSetLinePatternInContext(DWTextLineStyle style, CGFloat width, CGFloat phase, CGContextRef context){
    ...
    } else if (pattern == DWTextLineStylePatternCircleDot) {
...
static void DWTextDrawBorderRects(CGContextRef context, CGSize size, DWTextBorder *border, NSArray *rects, BOOL isVertical) {
    if (rects.count == 0) return;
    
    DWTextShadow *shadow = border.shadow;
    if (shadow.color) {
        CGContextSaveGState(context);
    ...
    for (NSValue *value in rects) {
        ...
        rect = DWTextCGRectPixelRound(rect);
    ...
    if (border.strokeColor && border.lineStyle > 0 && border.strokeWidth > 0) {
        ...
        DWTextSetLinePatternInContext(border.lineStyle, border.strokeWidth, 0, context);
        CGFloat inset = -border.strokeWidth * 0.5;
        if ((border.lineStyle & 0xFF) == DWTextLineStyleThick) {
            inset *= 2;
        ...
        //------------------------- second line ------------------------------//
        if ((border.lineStyle & 0xFF) == DWTextLineStyleDouble) {
            CGContextSaveGState(context);
...
static void DWTextDrawLineStyle(CGContextRef context, CGFloat length, CGFloat lineWidth, DWTextLineStyle style, CGPoint position, CGColorRef color, BOOL isVertical) {
    ...
    CGContextSaveGState(context); {
        if (isVertical) {
            CGFloat x, y1, y2, w;
            y1 = DWTextCGFloatPixelRound(position.y);
            y2 = DWTextCGFloatPixelRound(position.y + length);
            w = (styleBase == DWTextLineStyleThick ? lineWidth * 2 : lineWidth);
            
            CGFloat linePixel = DWTextCGFloatToPixel(w);
            if (fabs(linePixel - floor(linePixel)) < 0.1) {
                int iPixel = linePixel;
                if (iPixel == 0 || (iPixel % 2)) { // odd line pixel
                    x = DWTextCGFloatPixelHalf(position.x);
                } else {
                    x = DWTextCGFloatPixelFloor(position.x);
                }
            } else {
                x = position.x;
            }
            ...
            DWTextSetLinePatternInContext(style, lineWidth, position.y, context);
            CGContextSetLineWidth(context, w);
            if (styleBase == DWTextLineStyleSingle) {
                CGContextMoveToPoint(context, x, y1);
                CGContextAddLineToPoint(context, x, y2);
                CGContextStrokePath(context);
            } else if (styleBase == DWTextLineStyleThick) {
                CGContextMoveToPoint(context, x, y1);
                CGContextAddLineToPoint(context, x, y2);
                CGContextStrokePath(context);
            } else if (styleBase == DWTextLineStyleDouble) {
            ...
            CGFloat x1, x2, y, w;
            x1 = DWTextCGFloatPixelRound(position.x);
            x2 = DWTextCGFloatPixelRound(position.x + length);
            w = (styleBase == DWTextLineStyleThick ? lineWidth * 2 : lineWidth);
            ...
            if (fabs(linePixel - floor(linePixel)) < 0.1) {
                int iPixel = linePixel;
                if (iPixel == 0 || (iPixel % 2)) { // odd line pixel
                    y = DWTextCGFloatPixelHalf(position.y);
                } else {
                    y = DWTextCGFloatPixelFloor(position.y);
                }
            } else {
                y = position.y;
            }
            ...
            CGContextSetStrokeColorWithColor(context, color);
            DWTextSetLinePatternInContext(style, lineWidth, position.x, context);
            CGContextSetLineWidth(context, w);
            if (styleBase == DWTextLineStyleSingle) {
...
static void DWTextDrawText(DWTextLayout *layout, CGContextRef context, CGSize size, CGPoint point, BOOL (^cancel)(void)) {
    CGContextSaveGState(context); {
        ...
        for (NSUInteger l = 0, lMax = lines.count; l < lMax; l++) {
            ...
            for (NSUInteger r = 0, rMax = CFArrayGetCount(runs); r < rMax; r++) {
                ...
                DWTextDrawRun(line, run, context, size, isVertical, lineRunRanges[r], verticalOffset);
            }
...
static void DWTextDrawBlockBorder(DWTextLayout *layout, CGContextRef context, CGSize size, CGPoint point, BOOL (^cancel)(void)) {
    CGContextSaveGState(context);
    CGContextTranslateCTM(context, point.x, point.y);
    ...
    for (NSInteger l = 0, lMax = lines.count; l < lMax; l++) {
        ...
        for (NSInteger r = 0, rMax = CFArrayGetCount(runs); r < rMax; r++) {
            ...
            DWTextBorder *border = attrs[DWTextBlockBorderAttributeName];
            if (!border) continue;
            
            NSUInteger lineStartIndex = line.index;
            while (lineStartIndex > 0) {
                if (((DWTextLine *)lines[lineStartIndex - 1]).row == line.row) lineStartIndex--;
                else break;
            }
            
            CGRect unionRect = CGRectZero;
            NSUInteger lineStartRow = ((DWTextLine *)lines[lineStartIndex]).row;
            NSUInteger lineContinueIndex = lineStartIndex;
            NSUInteger lineContinueRow = lineStartRow;
            do {
                DWTextLine *one = lines[lineContinueIndex];
                ...
                DWTextLine *next = lines[lineContinueIndex + 1];
                if (next.row != lineContinueRow) {
                    DWTextBorder *nextBorder = [layout.text DW_attribute:DWTextBlockBorderAttributeName atIndex:next.range.location];
            ...
            DWTextDrawBorderRects(context, size, border, @[[NSValue valueWithCGRect:unionRect]], isVertical);
...
static void DWTextDrawBorder(DWTextLayout *layout, CGContextRef context, CGSize size, CGPoint point, DWTextBorderType type, BOOL (^cancel)(void)) {
    CGContextSaveGState(context);
    CGContextTranslateCTM(context, point.x, point.y);
    ...
    NSArray *lines = layout.lines;
    NSString *borderKey = (type == DWTextBorderTypeNormal ? DWTextBorderAttributeName : DWTextBackgroundBorderAttributeName);
    ...
    for (NSInteger l = 0, lMax = lines.count; l < lMax; l++) {
        ...
        for (NSInteger r = 0, rMax = CFArrayGetCount(runs); r < rMax; r++) {
            ...
            NSDictionary *attrs = (id)CTRunGetAttributes(run);
            DWTextBorder *border = attrs[borderKey];
            if (!border) continue;
            ...
            for (NSInteger ll = l; ll < lMax; ll++) {
                ...
                for (NSInteger rr = (ll == l) ? r : 0, rrMax = CFArrayGetCount(iRuns); rr < rrMax; rr++) {
                    ...
                    DWTextBorder *iBorder = iAttrs[borderKey];
                    ...
                    if (isVertical) {
                        DWTEXT_SWAP(iRunPosition.x, iRunPosition.y);
            ...
            for (NSInteger re = 0, reMax = runRects.count; re < reMax; re++) {
                ...
                if (isVertical) {
                    if (fabs(rect.origin.x - curRect.origin.x) < 1) {
                        curRect = DWTextMergeRectInSameLine(rect, curRect, isVertical);
                    } else {
            ...
            DWTextDrawBorderRects(context, size, border, drawRects, isVertical);
...
static void DWTextDrawDecoration(DWTextLayout *layout, CGContextRef context, CGSize size, CGPoint point, DWTextDecorationType type, BOOL (^cancel)(void)) {
    NSArray *lines = layout.lines;
    ...
    for (NSUInteger l = 0, lMax = layout.lines.count; l < lMax; l++) {
        ...
        for (NSUInteger r = 0, rMax = CFArrayGetCount(runs); r < rMax; r++) {
            ...
            NSDictionary *attrs = (id)CTRunGetAttributes(run);
            DWTextDecoration *underline = attrs[DWTextUnderlineAttributeName];
            DWTextDecoration *strikethrough = attrs[DWTextStrikethroughAttributeName];
            
            BOOL needDrawUnderline = NO, needDrawStrikethrough = NO;
            if ((type & DWTextDecorationTypeUnderline) && underline.style > 0) {
                needDrawUnderline = YES;
            }
            if ((type & DWTextDecorationTypeStrikethrough) && strikethrough.style > 0) {
            ...
            if (DWTextIsLinebreakString(runStr)) continue; // may need more checks...
            
            CGFloat xHeight, underlinePosition, lineThickness;
            DWTextGetRunsMaxMetric(runs, &xHeight, &underlinePosition, &lineThickness);
                ...
                while (shadow) {
                    ...
                    CGContextSaveGState(context); {
                        ...
                        CGContextSaveGState(context); {
                            ...
                            DWTextDrawLineStyle(context, length, thickness, underline.style, underlineStart, color, isVertical);
                        } CGContextRestoreGState(context);
                    } CGContextRestoreGState(context);
                    shadow = shadow.subShadow;
                }
            ...
            if (needDrawStrikethrough) {
                ...
                DWTextDrawLineStyle(context, length, thickness, strikethrough.style, strikethroughStart, color, isVertical);
            }
        }
    }
    CGContextRestoreGState(context);
...
static void DWTextDrawAttachment(DWTextLayout *layout, CGContextRef context, CGSize size, CGPoint point, UIView *targetView, CALayer *targetLayer, BOOL (^cancel)(void)) {
    
    BOOL isVertical = layout.container.verticalForm;
    CGFloat verticalOffset = isVertical ? (size.width - layout.container.size.width) : 0;
    
    for (NSUInteger i = 0, max = layout.attachments.count; i < max; i++) {
        DWTextAttachment *a = layout.attachments[i];
        ...
        rect = DWTextCGRectFitWithContentMode(rect, asize, a.contentMode);
        rect = DWTextCGRectPixelRound(rect);
        rect = CGRectStandardize(rect);
...
static void DWTextDrawShadow(DWTextLayout *layout, CGContextRef context, CGSize size, CGPoint point, BOOL (^cancel)(void)) {
    ...
    CGContextSaveGState(context); {
        ...
        for (NSUInteger l = 0, lMax = layout.lines.count; l < lMax; l++) {
            ...
            for (NSUInteger r = 0, rMax = CFArrayGetCount(runs); r < rMax; r++) {
                ...
                NSDictionary *attrs = (id)CTRunGetAttributes(run);
                DWTextShadow *shadow = attrs[DWTextShadowAttributeName];
                DWTextShadow *nsShadow = [DWTextShadow shadowWithNSShadow:attrs[NSShadowAttributeName]]; // NSShadow compatible
                if (nsShadow) {
...
static void DWTextDrawInnerShadow(DWTextLayout *layout, CGContextRef context, CGSize size, CGPoint point, BOOL (^cancel)(void)) {
    CGContextSaveGState(context);
    CGContextTranslateCTM(context, point.x, point.y);
    CGContextTranslateCTM(context, 0, size.height);
    CGContextScaleCTM(context, 1, -1);
    ...
    for (NSUInteger l = 0, lMax = lines.count; l < lMax; l++) {
        ...
        for (NSUInteger r = 0, rMax = CFArrayGetCount(runs); r < rMax; r++) {
            ...
            CGContextSetTextPosition(context, linePosX, linePosY);
            NSDictionary *attrs = (id)CTRunGetAttributes(run);
            DWTextShadow *shadow = attrs[DWTextInnerShadowAttributeName];
            while (shadow) {
                if (!shadow.color) {
                    shadow = shadow.subShadow;
                    continue;
...
static void DWTextDrawDebug(DWTextLayout *layout, CGContextRef context, CGSize size, CGPoint point, DWTextDebugOption *op) {
    UIGraphicsPushContext(context);
    CGContextSaveGState(context);
    CGContextTranslateCTM(context, point.x, point.y);
    CGContextSetLineWidth(context, 1.0 / DWTextScreenScale());
    ...
    if (op.CTFrameBorderColor || op.CTFrameFillColor) {
        ...
        if (!path) {
            ...
            if (op.CTFrameBorderColor) rect = DWTextCGRectPixelHalf(rect);
            else rect = DWTextCGRectPixelRound(rect);
    ...
    for (NSUInteger l = 0, lMax = lines.count; l < lMax; l++) {
        ...
        if (op.CTLineFillColor) {
            ...
            CGContextAddRect(context, DWTextCGRectPixelRound(lineBounds));
            CGContextFillPath(context);
        }
        if (op.CTLineBorderColor) {
            [op.CTLineBorderColor setStroke];
            CGContextAddRect(context, DWTextCGRectPixelHalf(lineBounds));
            CGContextStrokePath(context);
        }
        if (op.baselineColor) {
            [op.baselineColor setStroke];
            if (isVertical) {
                CGFloat x = DWTextCGFloatPixelHalf(line.position.x);
                CGFloat y1 = DWTextCGFloatPixelHalf(line.top);
                CGFloat y2 = DWTextCGFloatPixelHalf(line.bottom);
                CGContextMoveToPoint(context, x, y1);
                CGContextAddLineToPoint(context, x, y2);
                CGContextStrokePath(context);
            } else {
                CGFloat x1 = DWTextCGFloatPixelHalf(lineBounds.origin.x);
                CGFloat x2 = DWTextCGFloatPixelHalf(lineBounds.origin.x + lineBounds.size.width);
                CGFloat y = DWTextCGFloatPixelHalf(line.position.y);
                CGContextMoveToPoint(context, x1, y);
                CGContextAddLineToPoint(context, x2, y);
                CGContextStrokePath(context);
            }
        ...
        if (op.CTLineNumberColor) {
            ...
            num.DW_color = op.CTLineNumberColor;
            num.DW_font = [UIFont systemFontOfSize:6];
        ...
        if (op.CTRunFillColor || op.CTRunBorderColor || op.CTRunNumberColor || op.CGGlyphFillColor || op.CGGlyphBorderColor) {
            ...
            for (NSUInteger r = 0, rMax = CFArrayGetCount(runs); r < rMax; r++) {
                ...
                if (op.CTRunFillColor) {
                    ...
                    CGContextAddRect(context, DWTextCGRectPixelRound(runTypoBounds));
                    CGContextFillPath(context);
                }
                if (op.CTRunBorderColor) {
                    [op.CTRunBorderColor setStroke];
                    CGContextAddRect(context, DWTextCGRectPixelHalf(runTypoBounds));
                    CGContextStrokePath(context);
                }
                if (op.CTRunNumberColor) {
                    ...
                    num.DW_color = op.CTRunNumberColor;
                    num.DW_font = [UIFont systemFontOfSize:6];
                ...
                if (op.CGGlyphBorderColor || op.CGGlyphFillColor) {
                    for (NSUInteger g = 0; g < glyphCount; g++) {
                        ...
                        if (op.CGGlyphFillColor) {
                            ...
                            CGContextAddRect(context, DWTextCGRectPixelRound(rect));
                            CGContextFillPath(context);
                        }
                        if (op.CGGlyphBorderColor) {
                            [op.CGGlyphBorderColor setStroke];
                            CGContextAddRect(context, DWTextCGRectPixelHalf(rect));
                            CGContextStrokePath(context);
                        }
                    }
                }
...
- (void)drawInContext:(CGContextRef)context
                ...
                point:(CGPoint)point
                 view:(UIView *)view
                layer:(CALayer *)layer
                debug:(DWTextDebugOption *)debug
                cancel:(BOOL (^)(void))cancel{
    @autoreleasepool {
        if (self.needDrawBlockBorder && context) {
            if (cancel && cancel()) return;
            DWTextDrawBlockBorder(self, context, size, point, cancel);
        }
        if (self.needDrawBackgroundBorder && context) {
            if (cancel && cancel()) return;
            DWTextDrawBorder(self, context, size, point, DWTextBorderTypeBackgound, cancel);
        }
        if (self.needDrawShadow && context) {
            if (cancel && cancel()) return;
            DWTextDrawShadow(self, context, size, point, cancel);
        }
        if (self.needDrawUnderline && context) {
            if (cancel && cancel()) return;
            DWTextDrawDecoration(self, context, size, point, DWTextDecorationTypeUnderline, cancel);
        }
        if (self.needDrawText && context) {
            if (cancel && cancel()) return;
            DWTextDrawText(self, context, size, point, cancel);
        }
        if (self.needDrawAttachment && (context || view || layer)) {
            if (cancel && cancel()) return;
            DWTextDrawAttachment(self, context, size, point, view, layer, cancel);
        }
        if (self.needDrawInnerShadow && context) {
            if (cancel && cancel()) return;
            DWTextDrawInnerShadow(self, context, size, point, cancel);
        }
        if (self.needDrawStrikethrough && context) {
            if (cancel && cancel()) return;
            DWTextDrawDecoration(self, context, size, point, DWTextDecorationTypeStrikethrough, cancel);
        }
        if (self.needDrawBorder && context) {
            if (cancel && cancel()) return;
            DWTextDrawBorder(self, context, size, point, DWTextBorderTypeNormal, cancel);
        }
        if (debug.needDrawDebug && context) {
            if (cancel && cancel()) return;
            DWTextDrawDebug(self, context, size, point, debug);
        }
    }
}
...
- (void)removeAttachmentFromViewAndLayer {
    NSAssert([NSThread isMainThread], @"This method must be called on the main thread");
    for (DWTextAttachment *a in self.attachments) {
