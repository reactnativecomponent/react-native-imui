...
static int _DWTextKeyboardViewFrameObserverKey;

/// Observer for view's frame/bounds/center/transform
@interface _DWTextKeyboardViewFrameObserver : NSObject
...
@implementation _DWTextKeyboardViewFrameObserver {
...
- (void)addToKeyboardView:(UIView *)keyboardView {
    ...
    if (_keyboardView) {
        ...
        objc_setAssociatedObject(_keyboardView, &_DWTextKeyboardViewFrameObserverKey, nil, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    }
    _keyboardView = keyboardView;
    if (keyboardView) {
        [self addFrameObserver];
    }
    objc_setAssociatedObject(keyboardView, &_DWTextKeyboardViewFrameObserverKey, self, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}
...
+ (instancetype)observerForView:(UIView *)keyboardView {
    ...
    return objc_getAssociatedObject(keyboardView, &_DWTextKeyboardViewFrameObserverKey);
}
...
@implementation DWTextKeyboardManager {
...
- (instancetype)init {
    @throw [NSException exceptionWithName:@"DWTextKeyboardManager init error" reason:@"Use 'defaultManager' to get instance." userInfo:nil];
    return [super init];
}
...
- (void)_initFrameObserver {
    ...
    _DWTextKeyboardViewFrameObserver *observer = [_DWTextKeyboardViewFrameObserver observerForView:keyboardView];
    if (!observer) {
        observer = [_DWTextKeyboardViewFrameObserver new];
...
+ (instancetype)defaultManager {
    static DWTextKeyboardManager *mgr = nil;
...
- (void)_notifyAllObservers {
    ...
    DWTextKeyboardTransition trans = {0};
    ...
    if (!CGRectEqualToRect(trans.toFrame, _fromFrame)) {
        for (id<DWTextKeyboardObserver> observer in _observers.copy) {
