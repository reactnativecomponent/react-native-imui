...
@protocol DWTextLinePositionModifier;
 ...
 */
extern const CGSize DWTextContainerMaxSize;

/**
 The DWTextContainer class defines a region in which text is laid out.
 DWTextLayout class uses one or more DWTextContainer objects to generate layouts.
 
 A DWTextContainer defines rectangular regions (`size` and `insets`) or
 ...
 Example:
     ...
     └─────────────────────────────┘
 */
@interface DWTextContainer : NSObject <NSCoding, NSCopying>
...
/// The constrained size. (if the size is larger than DWTextContainerMaxSize, it will be clipped)
...
@property DWTextTruncationType truncationType;
...
@property (nullable, copy) id<DWTextLinePositionModifier> linePositionModifier;
@end


/**
 The DWTextLinePositionModifier protocol declares the required method to modify
 the line position in text layout progress. See `DWTextLinePositionSimpleModifier` for example.
 */
@protocol DWTextLinePositionModifier <NSObject, NSCopying>
@required
/**
 This method will called before layout is completed. The method should be thread-safe.
 @param lines     An array of DWTextLine.
 @param text      The full text.
 @param container The layout container.
 */
- (void)modifyLines:(NSArray<DWTextLine *> *)lines fromText:(NSAttributedString *)text inContainer:(DWTextContainer *)container;
@end


/**
 A simple implementation of `DWTextLinePositionModifier`. It can fix each line's position
 to a specified value, lets each line of height be the same.
 */
@interface DWTextLinePositionSimpleModifier : NSObject <DWTextLinePositionModifier>
...
/**
 DWTextLayout class is a readonly class stores text layout result.
 ...
 example: (layout with a circle exclusion path)
     ...
     └──────────────────────────┘
 */
@interface DWTextLayout : NSObject <NSCoding>
...
*/
+ (nullable DWTextLayout *)layoutWithContainerSize:(CGSize)size text:(NSAttributedString *)text;
...
/**
 ...
 @return A new layout, or nil when an error occurs.
 */
+ (nullable DWTextLayout *)layoutWithContainer:(DWTextContainer *)container text:(NSAttributedString *)text;
...
/**
 ...
 @return A new layout, or nil when an error occurs.
 */
+ (nullable DWTextLayout *)layoutWithContainer:(DWTextContainer *)container text:(NSAttributedString *)text range:(NSRange)range;

/**
 Generate layouts with the given containers and text.
 
 @param containers An array of DWTextContainer object (if nil, returns nil).
 @param text       The text (if nil, returns nil).
 @return An array of DWTextLayout object (the count is same as containers),
    or nil when an error occurs.
 */
+ (nullable NSArray<DWTextLayout *> *)layoutWithContainers:(NSArray<DWTextContainer *> *)containers
...
///< The text container
@property (nonatomic, strong, readonly) DWTextContainer *container;
///< The full text
...
///< Array of `DWTextLine`, no truncated
@property (nonatomic, strong, readonly) NSArray<DWTextLine *> *lines;
///< DWTextLine with truncated token, or nil
@property (nullable, nonatomic, strong, readonly) DWTextLine *truncatedLine;
///< Array of `DWTextAttachment`
@property (nullable, nonatomic, strong, readonly) NSArray<DWTextAttachment *> *attachments;
 ...
 */
- (nullable DWTextPosition *)closestPositionToPoint:(CGPoint)point;
...
/**
 ...
 @return A text position, or nil if not found.
 */
- (nullable DWTextPosition *)positionForPoint:(CGPoint)point
 ...
 */
- (nullable DWTextRange *)textRangeAtPoint:(CGPoint)point;
...
/**
 ...
 at point. Or nil if not found.
 */
- (nullable DWTextRange *)closestTextRangeAtPoint:(CGPoint)point;
 ...
 */
- (nullable DWTextRange *)textRangeByExtendingPosition:(DWTextPosition *)position;
...
/**
 ...
 @return An array of `DWTextSelectionRect` objects that encompass the selection.
 If not found, the array is empty.
 */
- (NSArray<DWTextSelectionRect *> *)selectionRectsForRange:(DWTextRange *)range;
...
/**
 ...
 If not found, the array is empty.
 */
- (NSArray<DWTextSelectionRect *> *)selectionRectsWithoutStartAndEndForRange:(DWTextRange *)range;
...
/**
 ...
 @param range An object representing a range in text.
 @return An array of `DWTextSelectionRect` objects contains the start and end to
 the selection. If not found, the array is empty.
 */
- (NSArray<DWTextSelectionRect *> *)selectionRectsWithOnlyStartAndEndForRange:(DWTextRange *)range;
...
- (void)drawInContext:(nullable CGContextRef)context
                ...
                debug:(nullable DWTextDebugOption *)debug
