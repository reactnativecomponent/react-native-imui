...
@implementation DWSelectionGrabberDot
...
@implementation DWSelectionGrabber

- (instancetype) initWithFrame:(CGRect)frame {
    self = [super initWithFrame:frame];
    if (!self) return nil;
    _dot = [[DWSelectionGrabberDot alloc] initWithFrame:CGRectMake(0, 0, 10, 10)];
    return self;
}

- (void)setDotDirection:(DWTextDirection)dotDirection {
    _dotDirection = dotDirection;
    [self addSubview:_dot];
    CGRect frame = _dot.frame;
    CGFloat ofs = 0.5;
    if (dotDirection == DWTextDirectionTop) {
        frame.origin.y = -frame.size.height + ofs;
        frame.origin.x = (self.bounds.size.width - frame.size.width) / 2;
    } else if (dotDirection == DWTextDirectionRight) {
        frame.origin.x = self.bounds.size.width - ofs;
        frame.origin.y = (self.bounds.size.height - frame.size.height) / 2;
    } else if (dotDirection == DWTextDirectionBottom) {
        frame.origin.y = self.bounds.size.height - ofs;
        frame.origin.x = (self.bounds.size.width - frame.size.width) / 2;
    } else if (dotDirection == DWTextDirectionLeft) {
...
- (CGRect)touchRect {
    ...
    if (_dotDirection == DWTextDirectionTop) {
        insets.top = -kTouchDotExtend;
    } else if (_dotDirection == DWTextDirectionRight) {
        insets.right = -kTouchDotExtend;
    } else if (_dotDirection == DWTextDirectionBottom) {
        insets.bottom = -kTouchDotExtend;
    } else if (_dotDirection == DWTextDirectionLeft) {
...
@interface DWTextSelectionView ()
@property (nonatomic, strong) NSTimer *caretTimer;
@property (nonatomic, strong) UIView *caretView;
@property (nonatomic, strong) DWSelectionGrabber *startGrabber;
@property (nonatomic, strong) DWSelectionGrabber *endGrabber;
@property (nonatomic, strong) NSMutableArray *markViews;
@end

@implementation DWTextSelectionView
...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    _startGrabber = [DWSelectionGrabber new];
    _startGrabber.dotDirection = DWTextDirectionTop;
    _startGrabber.hidden = YES;
    _endGrabber = [DWSelectionGrabber new];
    _endGrabber.dotDirection = DWTextDirectionBottom;
...
- (void)_startBlinks {
    [_caretTimer invalidate];
    if (_caretVisible) {
        _caretTimer = [NSTimer timerWithTimeInterval:kBlinkDuration target:[DWTextWeakProxy proxyWithTarget:self] selector:@selector(_doBlink) userInfo:nil repeats:YES];
...
- (void)setVerticalForm:(BOOL)verticalForm {
    if (_verticalForm != verticalForm) {
        ...
        self.startGrabber.dotDirection = verticalForm ? DWTextDirectionRight : DWTextDirectionTop;
        self.endGrabber.dotDirection = verticalForm ? DWTextDirectionLeft : DWTextDirectionBottom;
    }
}
...
- (CGRect)_standardCaretRect:(CGRect)caretRect {
    ...
    caretRect = DWTextCGRectPixelRound(caretRect);
...
- (void)setSelectionRects:(NSArray *)selectionRects {
    ...
    [selectionRects enumerateObjectsUsingBlock: ^(DWTextSelectionRect *r, NSUInteger idx, BOOL *stop) {
        CGRect rect = r.rect;
...
- (BOOL)isStartGrabberContainsPoint:(CGPoint)point {
    ...
    if (CGRectIntersectsRect(startRect, endRect)) {
        CGFloat distStart = DWTextCGPointGetDistanceToPoint(point, DWTextCGRectGetCenter(startRect));
        CGFloat distEnd = DWTextCGPointGetDistanceToPoint(point, DWTextCGRectGetCenter(endRect));
...
- (BOOL)isSelectionRectsContainsPoint:(CGPoint)point {
    ...
    for (DWTextSelectionRect *rect in _selectionRects) {
