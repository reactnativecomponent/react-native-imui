...
@implementation DWTextSimpleMarkdownParser {
    UIFont *_font;
...
- (void)_updateFonts {
    ...
    _boldFont = DWTextFontWithBold(_font);
    _italicFont = DWTextFontWithItalic(_font);
    _boldItalicFont = DWTextFontWithBoldItalic(_font);
...
- (void)setColorWithBrightTheme {
    ...
    _border = [DWTextBorder new];
    _border.lineStyle = DWTextLineStyleSingle;
    ...
    _border.strokeWidth = DWTextCGFloatFromPixel(1);
}
...
- (BOOL)parseText:(NSMutableAttributedString *)text selectedRange:(NSRangePointer)range {
    ...
    [text DW_removeAttributesInRange:NSMakeRange(0, text.length)];
    text.DW_font = _font;
    text.DW_color = _textColor;
    ...
    [_regexHeader enumerateMatchesInString:str options:0 range:NSMakeRange(0, str.length) usingBlock:^(NSTextCheckingResult *result, NSMatchingFlags flags, BOOL *stop) {
        ...
        [text DW_setColor:_controlTextColor range:NSMakeRange(r.location, whiteLen + sharpLen)];
        [text DW_setColor:_headerTextColor range:NSMakeRange(r.location + whiteLen + sharpLen, r.length - whiteLen - sharpLen)];
        [text DW_setFont:_headerFonts[sharpLen - 1] range:result.range];
    }];
    ...
    [_regexH1 enumerateMatchesInString:str options:0 range:NSMakeRange(0, str.length) usingBlock:^(NSTextCheckingResult *result, NSMatchingFlags flags, BOOL *stop) {
        ...
        if (linebreak.location != NSNotFound) {
            [text DW_setColor:_headerTextColor range:NSMakeRange(r.location, linebreak.location - r.location)];
            [text DW_setFont:_headerFonts[0] range:NSMakeRange(r.location, linebreak.location - r.location + 1)];
            [text DW_setColor:_controlTextColor range:NSMakeRange(linebreak.location + linebreak.length, r.location + r.length - linebreak.location - linebreak.length)];
        }
    }];
    ...
    [_regexH2 enumerateMatchesInString:str options:0 range:NSMakeRange(0, str.length) usingBlock:^(NSTextCheckingResult *result, NSMatchingFlags flags, BOOL *stop) {
        ...
        if (linebreak.location != NSNotFound) {
            [text DW_setColor:_headerTextColor range:NSMakeRange(r.location, linebreak.location - r.location)];
            [text DW_setFont:_headerFonts[1] range:NSMakeRange(r.location, linebreak.location - r.location + 1)];
            [text DW_setColor:_controlTextColor range:NSMakeRange(linebreak.location + linebreak.length, r.location + r.length - linebreak.location - linebreak.length)];
        }
    }];
    ...
    [_regexBreakline enumerateMatchesInString:str options:0 range:NSMakeRange(0, str.length) usingBlock:^(NSTextCheckingResult *result, NSMatchingFlags flags, BOOL *stop) {
        [text DW_setColor:_controlTextColor range:result.range];
    }];
    ...
    [_regexEmphasis enumerateMatchesInString:str options:0 range:NSMakeRange(0, str.length) usingBlock:^(NSTextCheckingResult *result, NSMatchingFlags flags, BOOL *stop) {
        NSRange r = result.range;
        [text DW_setColor:_controlTextColor range:NSMakeRange(r.location, 1)];
        [text DW_setColor:_controlTextColor range:NSMakeRange(r.location + r.length - 1, 1)];
        [text DW_setFont:_italicFont range:NSMakeRange(r.location + 1, r.length - 2)];
    }];
    ...
    [_regexStrong enumerateMatchesInString:str options:0 range:NSMakeRange(0, str.length) usingBlock:^(NSTextCheckingResult *result, NSMatchingFlags flags, BOOL *stop) {
        NSRange r = result.range;
        [text DW_setColor:_controlTextColor range:NSMakeRange(r.location, 2)];
        [text DW_setColor:_controlTextColor range:NSMakeRange(r.location + r.length - 2, 2)];
        [text DW_setFont:_boldFont range:NSMakeRange(r.location + 2, r.length - 4)];
    }];
    ...
    [_regexStrongEmphasis enumerateMatchesInString:str options:0 range:NSMakeRange(0, str.length) usingBlock:^(NSTextCheckingResult *result, NSMatchingFlags flags, BOOL *stop) {
        NSRange r = result.range;
        [text DW_setColor:_controlTextColor range:NSMakeRange(r.location, 3)];
        [text DW_setColor:_controlTextColor range:NSMakeRange(r.location + r.length - 3, 3)];
        [text DW_setFont:_boldItalicFont range:NSMakeRange(r.location + 3, r.length - 6)];
    }];
    ...
    [_regexUnderline enumerateMatchesInString:str options:0 range:NSMakeRange(0, str.length) usingBlock:^(NSTextCheckingResult *result, NSMatchingFlags flags, BOOL *stop) {
        ...
        [text DW_setColor:_controlTextColor range:NSMakeRange(r.location, 2)];
        [text DW_setColor:_controlTextColor range:NSMakeRange(r.location + r.length - 2, 2)];
        [text DW_setTextUnderline:[DWTextDecoration decorationWithStyle:DWTextLineStyleSingle width:@1 color:nil] range:NSMakeRange(r.location + 2, r.length - 4)];
    }];
    ...
    [_regexStrikethrough enumerateMatchesInString:str options:0 range:NSMakeRange(0, str.length) usingBlock:^(NSTextCheckingResult *result, NSMatchingFlags flags, BOOL *stop) {
        ...
        [text DW_setColor:_controlTextColor range:NSMakeRange(r.location, 2)];
        [text DW_setColor:_controlTextColor range:NSMakeRange(r.location + r.length - 2, 2)];
        [text DW_setTextStrikethrough:[DWTextDecoration decorationWithStyle:DWTextLineStyleSingle width:@1 color:nil] range:NSMakeRange(r.location + 2, r.length - 4)];
    }];
    ...
    [_regexInlineCode enumerateMatchesInString:str options:0 range:NSMakeRange(0, str.length) usingBlock:^(NSTextCheckingResult *result, NSMatchingFlags flags, BOOL *stop) {
        ...
        [text DW_setColor:_controlTextColor range:NSMakeRange(r.location, len)];
        [text DW_setColor:_controlTextColor range:NSMakeRange(r.location + r.length - len, len)];
        [text DW_setColor:_inlineTextColor range:NSMakeRange(r.location + len, r.length - 2 * len)];
        [text DW_setFont:_monospaceFont range:r];
        [text DW_setTextBorder:_border.copy range:r];
    }];
    ...
    [_regexLink enumerateMatchesInString:str options:0 range:NSMakeRange(0, str.length) usingBlock:^(NSTextCheckingResult *result, NSMatchingFlags flags, BOOL *stop) {
        NSRange r = result.range;
        [text DW_setColor:_linkTextColor range:r];
    }];
    ...
    [_regexLinkRefer enumerateMatchesInString:str options:0 range:NSMakeRange(0, str.length) usingBlock:^(NSTextCheckingResult *result, NSMatchingFlags flags, BOOL *stop) {
        NSRange r = result.range;
        [text DW_setColor:_controlTextColor range:r];
    }];
    ...
    [_regexCodeBlock enumerateMatchesInString:str options:0 range:NSMakeRange(0, str.length) usingBlock:^(NSTextCheckingResult *result, NSMatchingFlags flags, BOOL *stop) {
        ...
        if (lenStart + lenEnd < r.length) {
            ...
            [text DW_setColor:_codeTextColor range:codeR];
            [text DW_setFont:_monospaceFont range:codeR];
            DWTextBorder *border = [DWTextBorder new];
            border.lineStyle = DWTextLineStyleSingle;
            ...
            border.strokeWidth = DWTextCGFloatFromPixel(2);
            [text DW_setTextBlockBorder:_border.copy range:codeR];
        }
    }];
...
@implementation DWTextSimpleEmoticonParser {
...
- (BOOL)parseText:(NSMutableAttributedString *)text selectedRange:(NSRangePointer)range {
    ...
    for (NSUInteger i = 0, max = matches.count; i < max; i++) {
        ...
        CTFontRef font = (__bridge CTFontRef)([text DW_attribute:NSFontAttributeName atIndex:oneRange.location]);
        if (font) fontSize = CTFontGetSize(font);
        NSMutableAttributedString *atr = [NSAttributedString DW_attachmentStringWithEmojiImage:emoticon fontSize:fontSize];
        [atr DW_setTextBackedString:[DWTextBackedString stringWithString:subStr] range:NSMakeRange(0, atr.length)];
        [text replaceCharactersInRange:oneRange withString:atr.string];
        [text DW_removeDiscontinuousAttributesInRange:NSMakeRange(oneRange.location, atr.length)];
        [text addAttributes:atr.DW_attributes range:NSMakeRange(oneRange.location, atr.length)];
